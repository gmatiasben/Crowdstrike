...
Write-Output "=== Installing Chrome Extension for All Users ==="
...
foreach ($userProfile in $userProfiles) {
    ...
    try {
        ...
        $userPolicyPath = "Registry::HKEY_USERS\$userSID\SOFTWARE\Policies\Google\Chrome\3rdparty\extensions\$extensionId\policy"
...
if (Test-Path $defaultUserHive) {
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            $defaultPolicyPath = "Registry::HKEY_LOCAL_MACHINE\TempDefaultUser\SOFTWARE\Policies\Google\Chrome\3rdparty\extensions\$extensionId\policy"
...
Write-Output "Extension will install when users launch Chrome."
Write-Output ""
Write-Output "For immediate effect on active sessions:"
Write-Output "1. Users must close ALL Chrome windows"
Write-Output "2. Restart Chrome"
