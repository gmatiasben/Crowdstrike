...
def identify_duplicates(full: list, model: dict):
    ...
    dupeids = []
    dupelist = []
    for host_A in list_A:
        record_A = loads(dumps(host_A))  # Quick deepcopy
        record_A.pop("mac_address")
        record_A.pop("first_seen")
        for host_B in list_B:
            record_B = loads(dumps(host_B))  # Quick deepcopy
            record_B.pop("mac_address")
            record_B.pop("first_seen")
            if record_A["hostname"] == record_B["hostname"]:
                if record_A["device_id"] != record_B["device_id"]:
                    time_A = datetime.strptime(record_A["last_seen"], '%Y-%m-%dT%H:%M:%SZ')
                    time_B = datetime.strptime(record_B["last_seen"], '%Y-%m-%dT%H:%M:%SZ')
                    diff_time = time_A - time_B
                    duplicate_hostname = record_A["hostname"] 
                    #print(f"Hostname: {duplicate_hostname}, Fecha A: {time_A}, Fecha B: {time_B}, Diff: {diff_time}, Diff_Day: {diff_time.days}")
                    # Number of days between last seen of duplicate records
                    if diff_time.days >= 1:
                        record_B["hostname"] = f"{record_B['hostname']} (DUPLICATE)"
                        dupeids.append(record_B["device_id"])
                        dupelist.append(record_B)
